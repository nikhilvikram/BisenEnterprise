...
const MyOrdersPage = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      ...
      default: return "text-warning"; // Processing/Shipped
    }
  };

  if (loading) return <div className="container mt-5 text-center"><h3>Loading your orders...</h3></div>;
                ...
                {order.items.map((item, index) => (
                  <div key={index} className="d-flex align-items-center mb-3 pb-3 border-bottom">
                    ...
                    <div className="flex-grow-1">
                      ...
                      <small className="text-muted">Qty: {item.qty} x ₹{item.price}</small>
                    </div>
